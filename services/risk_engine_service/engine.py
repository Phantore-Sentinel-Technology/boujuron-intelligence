...
def analyze_event(event, behavior=None):
    ...
    if average_amount > 0 and amount >= max(average_amount * 5, average_amount + 100_000):
    ...
    if location and known_locations and location not in known_locations:
        signals.append(_signal("BEHAVIOR", "New transaction location", 20, f"Location {location.title()} is not in the user's history"))
    ...
    if device_identity and known_devices and device_identity not in known_devices:
        signals.append(_signal("BEHAVIOR", "New device", 25, f"Device {device_id or device} has not been seen for this user"))
    ...
    if timestamp:
        ...
        if usual_start is not None and usual_end is not None and not int(usual_start) <= hour <= int(usual_end):
            signals.append(_signal("BEHAVIOR", "Activity outside normal hours", 20, f"Activity at {hour:02d}:00 is outside the user's normal window"))
        elif hour < 4:
